...
class Dataset(object):
    def __init__(self, config):
        ...
        self.study_id_train = []
        self.study_id_val = []
        self.patient_id_train = []
        self.patient_id_val = []
        self.study_date_train = []
        self.study_date_val = []
        self.age_train = []
        self.age_val = []
        self.sex_train = []
    ...
    def load_padchest_df(self, verbose=0):
        ...
        # Load data from .csv
        filename = self.dataset_path + 'PADCHEST_chest_x_ray_images_labels_160K_01.02.19.csv'
        self.df = pd.read_csv(filename,
                              converters={'WindowCenter_DICOM': convert_dtype,
                                          'WindowWidth_DICOM': convert_dtype})

        # Drop images fron non downloaded folders
        downloaded_folders = [f.name for f in os.scandir(self.dataset_path) if f.is_dir() and not f.name.startswith('.')]                
    ...
    def load_data(self, verbose=0):
        ...
        for i, row in pbar:
            img_path = self.dataset_path + str(row.ImageDir) + '/' + row.ImageID
            pbar.set_description("Processing " + str(i) + "/" + str(total_imgs))
            ...
            study_id_list.append(str(row.StudyID))
            patient_id_list.append(str(row.PatientID))
        ...
        # Binarize label list
        print('# Loading binarizing data')
        binarizer = MultiLabelBinarizer(classes=self.labels)
        ...
        self.study_id_train, self.study_id_val = split_list(study_id_list, self.val_perc)
        self.patient_id_train, self.patient_id_val = split_list(patient_id_list, self.val_perc)
        self.study_date_train, self.study_date_val = split_list(study_date_list, self.val_perc)
        self.age_train, self.age_val = split_list(age_list, self.val_perc)
        self.sex_train, self.sex_val = split_list(sex_list, self.val_perc)
